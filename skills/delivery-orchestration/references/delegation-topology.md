...
The Sol Medium root is depth 0. Every custom profile is a terminal depth 1 leaf
that reports directly to the root. `max_depth = 1` is both the configured and
...
The configured efforts are fixed per profile. Route by task shape and escalate
the model rather than changing effort ad hoc: Spark fast path, Luna default,
then Sol only for difficulty or consequential risk.
