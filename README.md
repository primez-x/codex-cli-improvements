...
5. Semantically merge only the root model/effort, `[agents]` scalar values and
   ...
   `skills.config` registrations from `config.toml`. Leave the target's
   permission, sandbox, and approval settings unchanged; this portable kit
   intentionally does not define those machine-local policies. Preserve every
   other unrelated local setting.
...
The portable kit intentionally leaves permission and approval policy in the
target's user configuration. Do not copy machine-specific permission settings
